...
void table::merge(int pocz, int sr, int kon)
{
	...
	for (i=pocz; i<=kon; i++) tab_pom[i]=_table[i];  // Skopiowanie danych do tablicy pomocniczej
	...
	while (i<=sr) _table[q++]=tab_pom[i++]; // Przeniesienie nie skopiowanych danych ze zbioru pierwszego w przypadku, gdy drugi zbiór się skończył
}